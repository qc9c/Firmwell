...
shellbuiltInWords = ['alias', 'bg', 'bind', 'builtin', 'caller', 'cd', 'compgen', 'complete', 'compopt', 'declare', \
                     'dirs', 'disown', 'echo', 'enable', 'exit', 'export', 'false', 'fc', 'fg', 'getopts', 'hash',
...
usualCommands = ['cat', 'dd', 'mkdir', 'rmdir', 'rm', '-rf', 'ls', 'ps', 'tree', 'find', 'grep', 'egrep', 'sed', 'awk',
                 'ifconfig', 'ping', 'rm', 'du', 'df', 'less', 'more', 'test', 'cp', 'sleep', 'chmod', 'mv', 'logger',
                 'head', 'reboot', 'mtd', 'rcS', 'rc', 'wget', 'sh', 'ssi', 'gunzip', 'lsmod', 'brctl', 'dmesg',
                 'mount', 'init', 'free_caches', 'echo', 'insmod', 'killall', 'time', 'date', 'ip', 'openssl', 'tc',
                 'ds', 'igmpproxy', 'rt2860apd', 'rtinicapd', 'ledcontrol', 'syslogd', 'route', 'udhcpc', 'rmmod',
                 'klogd', 'sysctl', 'datalib', 'crond', 'wc', 'cut', 'hexdump', 'tail', 'tr', 'uniq', 'sort', 'diff',
                 'cmp', 'tee', 'split', 'comm', 'join', 'nl', 'paste', 'seq', 'shred', 'stat', 'sync', 'touch',
                 'truncate', 'uptime', 'wc', 'who', 'whoami', 'yes', 'zcat', 'zless', 'zmore', 'zgrep', 'zdiff', 'zcmp',
                 'zforce', 'zegrep', 'zfgrep', 'zless', 'zmore', 'znew', 'zcat', 'zdiff', 'zgrep', 'zless', 'zmore',
                 'znew', 'zforce', 'zegrep', 'zfgrep', 'zless', 'zmore', 'znew', 'zforce', 'zegrep', 'zfgrep', 'zless',
                 'zmore', 'znew', 'zforce', 'zegrep', 'zfgrep', 'zless', 'zmore', 'znew', 'zforce', 'zegrep', 'zfgrep',
                 'zless', 'zmore', 'znew', 'zforce', 'zegrep', 'zfgrep', 'zless', 'zmore', 'znew', 'zforce', 'zegrep',
                 'zfgrep', 'zless', 'zmore', 'znew', 'zforce', 'zegrep', 'zfgrep', 'zless', 'zmore', 'znew', 'zforce',
                 'zegrep', 'zfgrep', 'zless', 'zmore', 'znew', 'zforce', 'zegrep', 'zfgrep', 'printf', 'arping',
                 'fgrep', 'kill', '[', 'ebtables', 'passwd', 'udhcpd', 'config', 'run-ramfs', 'iptables', 'ip6tables',
                 'ip6tables-restore', 'ip6tables-save', 'iptables-restore', 'ip', 'iwpriv']
complexWords = ['for', 'if', 'else', 'elif', 'fi', 'do', 'done', 'while', 'case', 'esac', 'until', 'select', 'break',
                'continue']  # {,},((,)),[[,]],
brackets = ['(', ')', '{', '}', '[', ']', '((', '))', '[[', ']]', '{{'  '}}', '()', '[]', '{}']
...
BashSimpleCommand = BashIdentifier.setResultsName("command") + BashFlag[...] + BashPath[...] + BashIdentifier[...] + \
                    BashRedirect[...]
BashCompoundCommand = BashSimpleCommand + (oneOf("&& & || | ;") + BashSimpleCommand)[1, ...]

# Message keywords
check_msg_list = ['incorrect_firmware', 'upg_upgrade_error', 'upg_release_note_3_4', 'current_firmware', 'new_firmware',
                  'chkupg', 'auto_upg_check', 'auto_upg_seconds', 'upg_md5_check_error', 'auto_update_header',
                  'info_mark_ver', 'upgrade_head', 'upgrade_select_disk', 'upgrade_mark', 'upgrade_new_version',
                  'upgrade_upon', 'upgrade_note1', 'upgrade_note2', 'autp_upg_head', 'auto_upg_check_ser',
                  'auto_upg_90s', 'auto_upg_nowan_head', 'auto_upg_nowan', 'auto_upg_detect', 'auto_upg_start_detect',
                  'auto_upg_not_display', 'autp_upg_firmware', 'upg_progess', 'upg_find_old', 'wait_upg_head',
                  'wait_serv', 'wait_serv1',
                  'wait_serv2', 'wait_serv3', 'wait_cancel', 'download_confile_fail', 'wait_return', 'wait_new_version',
                  'no_new_version', 'wait_download', 'upg_download_error', 'upg_upload_error', 'wait_string',
                  'line_string', 'failure_head', 'old_ver', 'new_ver', 'upload_ver', 'download_image',
                  'download_image_fail', 'not_img', 'error_module', 'upgrade_1', 'upgrade_2', 'upgrade_3',
                  'error_firm_reg', 'upgrade_4', 'upgrade_5', 'upgrade_6', 'upgrade_7', 'upgrade_8', 'upgrade_9',
                  'upgrade_10', 'upgrade_11', 'upgrade_12', 'oldver1', 'oldver2', 'oldver3', 'upgrade_turnoff_auto',
                  'in_upgrade', 'invalid_filename', 'firm_upgrade',
                  'The router cannot connect to ASUS server to check for the signature update. After reconnecting to the Internet, go back to this page and click Check to check for the latest signature updates',
                  'Signature checking ...', 'Signature is up to date', 'Signature update failed',
                  'Signature is updating', 'Signature update completely', 'The F/W is updating ...<br><br>',
                  'Please <font color=red><b>DO NOT POWER OFF</b></font> the device.<br><br>', 'And please wait for',
                  ...
                  'seconds...', 'Please choose a file to upgrade!',
                  'The selected file is in wrong format, please select another.',
                  'Are you sure to upgrade the firmware?',
                  'The device cannot perform software upgrades via the remote management tool. Please upgrade software from within the LAN.',
                  'Firmware Version:', 'Hardware Version:', 'Firmware Upgrade', 'Processing...',
                  'Please wait until system reboots...', 'tf_FWF1', 'tf_msg_FWUgReset', 'tf_really_FWF',
                  '_upgrade_firmw', '_FIRMW_DESC', '_FIRMW_DESC_sub', '_upgrade_firmw',
                  'Do you want to upgrade Firmware?', "Estimated upgrade time:", "Launching firmware upgrade in",
                  "Invalid firmware file", "0 <firmware file> [<delay>]", "About to run firmware update",
                  "Firmware update file check failed!", "Firmware update file"]


def parseAssignmentStatement(stmt):
    assignmentCommands = oneOf("export alias let local")
    assignmentExpr = ZeroOrMore(assignmentCommands) + BashIdentifier.setResultsName("lhs") + "=" + (
                BashIdentifier | BashNumber).setResultsName("rhs")
    try:
...
def parseCommandStatement(stmt):
    ...
    assignmentExpr = OneOrMore(assignmentCommands).setResultsName("lhs") + BashFlag[...] + (
        BashIdentifier).setResultsName("rhs")
    try:
        tokens = assignmentExpr.parseString(stmt)
...
def parseInlineComments(stmt):
    ...
    assignmentExpr = OneOrMore(assignmentCommands).setResultsName("lhs") + BashFlag + (BashIdentifier).setResultsName(
        "rhs")
    try:
        tokens = assignmentExpr.parseString(stmt)
        return tokens.lhs + tokens.rhs
...
def readScriptFile(filePath):
    ...
    content = [line.strip() for line in content if
               (re.search("^[\s+]*#", line) == None) and (re.match(r'^\s*$', line) == None)]
    return content
...
def shell_analysis(path, callPath):
    ...
    for line in lines:
        ...
        if flag == "FUNCBLOCK":
            flag, stack, deps, calls, execs, blocks[-1], checks, end = blockParser(stack, currentCmd, calls, execs,
                                                                                   blocks[-1], checks, shs)
            if end == 1:
                continue
            ...
            if key != 'metadata' and key != 'main':
                for deps in results[file][key]['depfunctions']:
                    if deps not in results[file]['main']['depfunctions']:
                        ...
                        results[file]['main']['depfunctions'][deps] = list(
                            set(results[file]['main']['depfunctions'][deps]) | set(
                                results[file][key]['depfunctions'][deps]))
