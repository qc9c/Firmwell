...
USER_AUTHS = ['admin', 'root', '']
PASSWORD_AUTHS = ['', 'password', 'admin', '1234', "Admin"]
MAX_RETRIES = 3
...
class URLCheck:
    ...
    def probe(self):
        ...
        while True:
            ...
            # RECHECK
        self.session.close()
        return self.last_status_code != -1, self.last_status_code
    ...
    def probe(self, ips, ports):
        ...
        for ip in ips:
            for port in ports:
                ...
                success, last_status_code = uc.probe()
                if success:
                    self.urlchecks.append(uc)
                if uc.wellformed is True:
                    return True
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("USAGE: http_check.py [BRAND] [URL;URL;URL]")
    brand = sys.argv[1]
    # brand = "tenda"
    ...
    if probe_success:
        success, wellformed, curlsuccess = checker.check(trace=None, exit_code=None, timedout=None,
