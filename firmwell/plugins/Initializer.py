                ...
                try:
                    ...
                    if "<html" in page_source or "<script" in page_source:
                        for entry in self.driver.get_log('browser'):
                            if entry["level"] == "SEVERE":
                                if HTTP_500_MSG in entry["message"]:
                                    ...
                                    isWhitelisted = False
                                    # if "jquery.min.js" in error_target: # TODO, use curl to determin it if exists, in manual test, it exists
                                    if ".js" in error_target: # TODO, use curl to determin it if exists, in manual test, it exists
                                        continue# for tplink, it seems work normal
                                    if "cgi-bin" in error_target and "luci" in error_target and len(page_source) > 300:
                                        break# in true chrome broser, the web server worked normally
                        ...
                        if "401" in raw_data.lower() and "unauthorized" in raw_data.lower():
                            if "enabling CAPTCHA Graphical Authentication provides added security protection to prevent unauthorized".lower() not in raw_data.lower():  
                                if "The information you entered does not match what is saved on the router. Please enter it again.".lower() not in raw_data.lower():
                                    return False, 401
...
class Login:
    ...
    def check_login_type(ip, brand):
        ...
        if r.status_code == 401:
            ...
            if brand == "dlink":  # dlink normal login
                ...
                return 'unknown'
        elif r.text.find('<div class="message">Login to the router : </div>') != -1: 
            return 'dlink_600'
        elif r.text.find("<meta http-equiv=Refresh content=\'0; url=index.php\'>") != -1:
            return 'dlink_dap2360'
            ...
            if r.text.find('/login/Auth') != -1:
                ...
                if r and r.status_code == 200:
                    if r.text == "1" or r.text == '0':
                        return 'tenda_auth_md5'
                    return 'tenda_auth'
    ...
    def login(session, brand, ip, login_type, username, password):
        ...
        if login_type == 'basic':
        ...
        elif login_type == 'dlink_600':
            login_cgi = "session.cgi"
            login_name = username
            log_pass = password
            ...
            headers["Referer"] = ip
            headers["Cache-Control"] = "max-age=0"
            headers['Cookie'] = "uid=OZwgnQPArm"  # fixed, calculate by front-end
            payload = {
                "REPORT_METHOD": "xml",
            ...
            new_headers = dict(reply.request.headers)
            if "Content-Length" in new_headers:
                new_headers.pop("Content-Length")
            return reply.status_code == 200, new_headers, reply.request.body, '{}/{}'.format(ip, login_cgi)

        elif login_type == 'dlink_dap2360':
            login_cgi = "login.php"
            login_name = username
            log_pass = password
            ...
            headers["Referer"] = ip
            headers["Cache-Control"] = "max-age=0"
            headers['Cookie'] = "Cookie: dlink_uid=sfzbmliycb" # # fixed, calculate by front-end
            payload = {
                "ACTION_POST": "LOGIN",
                "f_date": "",
                "f_time": "",
                "LOGIN_USER": login_name,
                "LOGIN_PASSWD": log_pass,
                "login": "+Login+",
            }
            reply = session.post('{}/{}'.format(ip, login_cgi), headers=headers, data=payload)
            ...
            # # 0828 tmp
            # # US_AC500V1BR_V1.0.0.14_en_TD, password: 'YWRtaW4=' -> 'YWRtaW4%3D'
            # import urllib.parse
            # log_pass = urllib.parse.quote(log_pass)
            ...
            if reply.status_code == 200:
                ...
                if "<html" in data and "/login/Auth" not in data:
                    ...
                    payload = {'mitUSERNAME': username, 'mitPASSWORD': log_pass} # US_AC500V1BR_V1.0.0.14_en_TD
                    reply = session.post('{}/{}'.format(ip, login_cgi), headers=headers, data=payload)
                    if reply.status_code == 200:
                        data = reply.text
                        if "<html" in data and "/login/Auth" not in data:
        ...
        elif login_type == 'tenda_auth_md5':
            # log_pass = base64.b64encode(password.encode('utf-8'))
            import hashlib
            log_pass = hashlib.md5(password.encode('u8')).hexdigest()
            # log_pass = password
            headers = requests.utils.default_headers()
            headers["User-Agent"] = "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/56.0.2924.76 Safari/537.36"
            headers["Origin"] = ip
            headers["Referer"] = ip
            payload = {'username':username,'password':log_pass}
            login_cgi = "login/Auth"
            reply = session.post('{}/{}'.format(ip, login_cgi), headers=headers, data=payload)
            success = False
            if reply.status_code == 200:
...
class Initializer:
    def __init__(self, brand, ip, port, username, password):
        ...
        if self.brand == "netgear":
            ...
            self.pattern = re.compile(r'(<div id="wizard_title">Welcome</div>|Welcome to the D-Link Setup Wizard)') 
        ...
        while not run_complete:
            try:
                ...
                if self.brand == "netgear":
                    ...
                    if "PAGE.OnClickCancel()" in page_source: # DIR 845
                        button = self.driver.find_element(By.NAME, 'b_exit')
                        button.click()
    ...
    def ClickNext(self):
        if self.driver.page_source.find('btnsContainer_div') != -1: # wnr2000v3, WNDR3800, JNR3210, R6200v2
            ...
            if isinstance(self.driver.find_elements(By.ID, 'btnsContainer_div'), list): 
                for i in self.driver.find_elements(By.ID, 'btnsContainer_div'):
                    i.click()
            ...
            if success:
                ...
                if len(raw_creds_dump_path) > 0:
                    with open(f"{raw_creds_dump_path}", "w") as rawcredFile:
                        ...
                        rawcredFile.write("\r\n")
                        rawcredFile.write(credentials['loginpayload'])
            else:
                print("Login failed")
        except Exception as e:
            print(f"[---] {str(e)}")
...
if __name__ == "__main__":
    ...
    os.environ['PATH'] = "/analysis" + ':' + os.environ['PATH']
