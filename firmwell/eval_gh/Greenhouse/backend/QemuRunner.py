...
class QemuRunner:
    ...
    @staticmethod
    def create_network_device(docker_manager, device_name, ip_address):
        """
        Create a network device in the Docker container and set its IP address, ensuring connectivity with the host network

        Args:
            docker_manager: Docker container manager instance
            device_name (str): network device name
        ...
        return NetworkUtil.create_network_device2(docker_manager, device_name, ip_address)
    ...
    def run(self, delay=60, timeout=HARD_TIMEOUT, extra_args="", nd_args="", bin_cwd="/",
            potential_urls=[], ports_file="", bg_cmds=[], bg_sleep=0, interface_cmds=[],
            mac="",
            has_ipv6=False, greenhouse_mode=True, tempCont=None, docker_manager=None, iface_ip_map=None):
        ...
        # fw
        original_qemu_arch = self.qemu_arch
        
        if not self.qemu_arch.endswith("-gh"):
            self.qemu_arch = f"{self.qemu_arch}-gh" 
            print(f"[GH] Using qemu with -gh suffix: {self.qemu_arch}")
        ...
        # Files.copy_directory(self.fs_path, dest) # shutil.Error: [('/tmp/greenhouse_fs/var/fuse', '/tmp/greencontainers/fs/var/fuse', '[Errno 1] Operation not permitted')]
        
        tempCont = tempCont
        
        # Copy scripts to container
        docker_manager.docker_cp_to_container(wrapper_script_path, "/fs/run.sh")
        docker_manager.docker_cp_to_container(clean_script_path, "/fs/run_clean.sh")
        docker_manager.docker_cp_to_container(bg_script_path, "/fs/bg_script.sh")
        docker_manager.docker_cp_to_container(setup_script_path, "/fs/setup_script.sh")
        docker_manager.docker_cp_to_container(command_script_path, "/fs/qemu_run.sh")
        ...
        # build_success = False
        # os.environ['DOCKER_HOST'] = 'tcp://127.0.0.1:2375' # fw
        ...
        # tempCont = self.client.containers.create(img, detach=False, tty=True, mem_limit="8G",
        #                                               ipc_mode="shareable", privileged=True)
        # tempCont = self.client.containers.create(img, detach=False, tty=True, mem_limit="8G",
        #                                          ipc_mode="shareable", privileged=False,
        #                                          cap_add = ["NET_ADMIN", "CAP_SYS_CHROOT"])
        ...
        try:
            ...
            #  # Here we should create the necessary network interfaces using docker_manager
            # if docker_manager is not None and len(interface_cmds) > 0:
            #     print(f"Creating network interfaces with commands: {interface_cmds}")
            # from backend.utils.NetworkUtil import create_network_device
                
            #     # Check if interface_cmds is a tuple, if so, unpack it
            #     if isinstance(interface_cmds, tuple) and len(interface_cmds) == 3:
            #         _, _, iface_ip_map = interface_cmds
            #         # Use iface_ip_map to create network devices
            #         if iface_ip_map:
            #             for iface_name, ip_addr in iface_ip_map.items():
            #                 print(f"Creating network device {iface_name} with IP {ip_addr}")
            #                 create_network_device(docker_manager, iface_name, ip_addr)
            #     # Backward compatible with old code, if interface_cmds is a list of strings
            #     else:
            #         print("Using legacy interface command handling")
            #         for cmd in interface_cmds:
            #             print(f"Processing command: {cmd}")
            #             # Old processing logic remains unchanged
            
            from firmwell.backend.utils.NetworkUtil import NetworkUtil
            if iface_ip_map is not None and len(iface_ip_map) > 0:
                for iface_name, ip_addr in iface_ip_map.items():
                    print(f"Creating network device {iface_name} with IP {ip_addr}")
                    # QemuRunner.create_network_device(docker_manager, iface_name, ip_addr)
                                
                    network_util = NetworkUtil(docker_manager)
                    network_util.create_network_device(iface_name, ip_addr)
            ...
            # Kill processes matching self.http_path,
            # delete all trace.log* under /fs
            # Kill processes matching http_path
            if self.relative_bin_path:
                try:
                    cmd = f"sh -c 'ps -ef | grep {self.bin_path} | grep -v grep | awk \"{{print \\$2}}\"'"
                    pids = tempCont.exec_run(cmd)[1].decode().strip().split('\n')
                    for pid in pids:
                        if pid:
                            print(f"[GH] Killing process {pid} ({self.relative_bin_path})")
                            tempCont.exec_run(f"sh -c 'kill -9 {pid}'")
                except Exception as e:
                    print(f"[GH] Error killing {self.relative_bin_path} processes: {e}")

            # Remove all trace.log files
            try:
                cmd = "sh -c 'rm -f /fs/trace.log*'"
                tempCont.exec_run(cmd)
                print("[GH] Removed trace.log files")
                
            except Exception as e:
                print(f"[GH] Error removing trace.log files: {e}")
