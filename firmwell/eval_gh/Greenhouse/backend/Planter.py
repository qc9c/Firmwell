...
class Fixer():
    ...
    def setup_devfiles(self, fs_path):
        ...
        Files.write_file(os.path.join(fs_path, "dev", "urandom"), RAND, root=fs_path,
                         silent=True)  # 'random' bytes for entropy
        Files.write_file(os.path.join(fs_path, "dev", "random"), RAND, root=fs_path,
                         silent=True)  # 'random' bytes for entropy
        if not os.path.exists(os.path.join(fs_path, "dev")):
            Files.mkdir(os.path.join(fs_path, "dev"), root=fs_path)
        if not os.path.exists(os.path.join(fs_path, "proc")):
            Files.mkdir(os.path.join(fs_path, "proc"), root=fs_path)
                        ...
                        if f.lower().endswith(filename.lower() + "." + tag):
                            ...
                            if os.path.islink(file_path):
                                ...
                                if not file_path.startswith(
                                        fs_path):  # handle symlinks that resolve to outside root folder
                                    while file_path.startswith("/") or file_path.endswith("/"):
                                        file_path = file_path.strip("/")
                                    file_path = os.path.join(fs_path, file_path)
                            if file_path in skip:
    ...
    def write_nvram(self, keys, changelog=[]):
        print("Writing nvram - using nvram path:", os.path.join(self.gh_path, "libnvram_faker", "conf"))
        nvramNames = []
        nvramValues = []
        for key in keys:
            if key in self.nvram_map:
                nvramNames.append(key)
                nvramValues.append(str(self.nvram_map[key]))
        for key in keys:
            key = key.strip().strip("/")
            if len(key) <= 0:
...
class Planter():
    ...
    def __init__(self, gh_path, scripts_path, qemu_src_path, brand):
        ...
        self.qemu_src_path = qemu_src_path
        # self.fixer = None
        self.fixer = Fixer(qemu_src_path, self.gh_path, self.scripts_path, brand, False)
    ...
    def check_cwd(self, fs_path, targets, old_cwd, cwd_rh_replaced, already_success):
        ...
        cwds_sorted = sorted(majority_cwds,
                             key=lambda x: ("www" not in x and "web" not in x and "htm" in x, x.count('/'), len(x), x))
    ...
    def add_interfaces(self, interfaces, urls):
        ...
        noninterface_urls = []
        iface_cmds = []
        iface_ip_map = {} # new, record network device to IP mapping
        ...
        for iface in interfaces:
            ...
            # New: record network device to IP mapping
            iface_ip_map[iface] = url
            print("iface_ip_map:", iface_ip_map)
            i += 1
        new_urls.extend(noninterface_urls)
        
        # Return the additional iface_ip_map
        return new_urls, iface_cmds, iface_ip_map
    ...
    def find_sourcefile(self, target, fs_path, path):
        ...
        if len(sourcefile) <= 0 and (target.endswith(".conf") or target.endswith(".cnf") or target.endswith(".config")):
            targetbasename = os.path.basename(target.rsplit(".")[0])
            sourcefiles = self.fixer.find_files_with_extension(targetbasename, [".config", ".conf", "cnf"], fs_path,
                                                               skip=[path])
            for sf in sourcefiles:
                if sf.endswith(targetbasename):
                    sourcefile = sf
    ...
    def transplant(self, fs_path, targets, folders, configs, failed, already_success, no_skip, hackdevproc, changelog):
        ...
        for target in targets:
            ...
            if target.endswith(".pid") and target not in no_skip:
                ...
                print("    - target is a PID file!")
                print("    - Creating file ", path)
                Files.touch_file(target, root=fs_path, silent=True)
        ...
        for root, dirs, files in os.walk(target_fs, topdown=False):
            for f in files:
                ...
                if os.path.exists(fpath) and fpath.startswith(target_fs):
                    ...
                    if stat.S_ISBLK(st_mode) or stat.S_ISCHR(st_mode) or stat.S_ISSOCK(st_mode) or stat.S_ISFIFO(
                            st_mode):
