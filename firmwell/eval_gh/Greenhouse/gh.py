...
class Greenhouse():
    ...
    def __init__(self, args):
        ...
        print("GH path init - GH base path:", self.ghbasepath)
        self.qemu_src_path = os.path.join(self.ghbasepath, "qemu")
        print("GH path init - QEMU source path:", self.qemu_src_path)
        self.gh_src_path = os.path.join(self.ghbasepath, "greenhouse_files")
        print("GH path init - GH files path:", self.gh_src_path)
        self.scripts_path = os.path.join(self.ghbasepath, "scripts")
        print("GH path init - scripts path:", self.scripts_path)
        self.analysis_path = os.path.join(self.ghbasepath, "analysis")
        print("GH path init - analysis path:", self.analysis_path)
        self.external_qemu = os.path.join(self.ghbasepath, "greenhouse_files", "external_qemu")
        print("GH path init - external QEMU path:", self.external_qemu)
    ...
    def get_checker(self):
        """Get the appropriate checker instance"""
        print("Getting checker - brand:", self.brand, "analysis path:", self.analysis_path)
    ...
    ## firmwell version
    # def patch_loop(self):
    def fw_patch_loop(self, new_run_args, cwdpath, checker, tempCont, docker_manager, greenhouse_patch_flag):
        # ======fw======

        # Get initial filesystem state
        # print("[GH] Collecting initial filesystem state...")
        # initial_fs_state = collect_file_system_state(self.fs_path)
        # print(f"[GH] Collected info for {len(initial_fs_state)} initial files")
        ...
        # Disable debug logging for angr and cle
        import logging
        ...
        """
        Use GH original logic for analysis and fixing, but apply operations to Firmwell's container

        Args:
            docker_manager: Firmwell's Docker manager
            trace_path: trace.log path from the container
            trace_json_path: generated trace.json path
            emulation_output: container output log, fetched from container if None
            baseline_mode: whether in baseline mode

        Returns:
            tuple: (targets, folders, configs, ip_addrs, ipv6_addrs, ports, interfaces, binary)
                  where binary is the patched binary object (if any)
        """
        print("Starting GH fix in Firmwell - base path:", self.ghbasepath)
        print("Starting GH fix in Firmwell - filesystem path:", self.fs_path)
        print("Starting GH fix in Firmwell - binary path:", self.bin_path)
        
        
        # 1. Collect current filesystem state
        initial_files = set()
        for root, dirs, files in os.walk(self.fs_path, topdown=False):
            for file in files:
                path = os.path.join(root, file)
                rel_path = os.path.relpath(path, self.fs_path)
                initial_files.add(rel_path)
        ...
        self.gh = Planter(gh_path=self.gh_src_path, scripts_path=self.scripts_path, qemu_src_path=self.qemu_src_path,
                          brand=self.brand)
        # setupresult = self.gh.setup_env(self.qemu_src_path, self.fs_path, self.bin_path, self.baseline)
        # self.gh.clean_fs(self.fs_path)
        
        # self.gh.fixer.setup_custom_libraries(self.fs_path)
        self.gh.fixer.initial_setup(self.fs_path, self.bin_path)  # read nvram conf
        ...
        # Copy libnvram-faker.so to the container
        print("[GH] Copying libnvram-faker.so to container...")
        self.copy_libnvram_faker_to_container(docker_manager, tempCont)
        
        
        # FW
        self.extra_args = new_run_args # update
        bin_cwd = cwdpath # update
        ...
        print("GH bin_path:", self.bin_path)
        print("GH extra_args:", self.extra_args)
        
        # No longer use bg process, otherwise it will overwrite xmldb
        ...
        # if len(ubusd_cmd) > 0:
        #     bg_cmds.append(ubusd_cmd)
        #     bg_cmds.append("/bin/sleep 2")
        ...
        while True:
            ...
            success, wellformed, connected = checker.check(trace, exit_code, timedout, False, self.strict)
            # success, wellformed, connected = checker.check(False, strict=True)
            ...
            if not greenhouse_patch_flag:
                ...
                # add new interfaces
                urls_with_interfaces, interface_cmds, iface_ip_map = self.gh.add_interfaces(interfaces, self.urls)
                print(f"\n\nurls_with_interfaces", urls_with_interfaces, interface_cmds, iface_ip_map )
                print("\n\n")
                ...
                if greenhouse_mode and (self.max_cycles < 0 or count < self.max_cycles):
                    ...
                    new_or_modified_files = current_files - initial_files
                    
                    # 8. Apply filesystem changes to the Firmwell container
                    for rel_path in new_or_modified_files:
                        basename = os.path.basename(rel_path)
                        if any(i in basename for i in ['trace.log', 'trace.json', 'npipes.log', "patch.log"]):
                            continue
                        
                        host_path = os.path.join(self.fs_path, rel_path)
                        container_path = os.path.join("/fs", rel_path)
                        
                        # Ensure target directory exists
                        parent_dir = os.path.dirname(container_path)
                        tempCont.exec_run(f"mkdir -p {parent_dir}")

                        # Copy file to container
                        tempCont.exec_run(f"rm -rf {container_path}") # acos_nat_cli
                        docker_manager.docker_cp_to_container(host_path, container_path)
                        
                    # # Use helper function to collect current filesystem state
                    # print("[GH] Collecting current filesystem state...")
                    # current_fs_state = collect_file_system_state(self.fs_path)
                    # print(f"[GH] Collected info for {len(current_fs_state)} current files")

                    # # Compare filesystem changes
                    # changed_files = compare_file_systems(initial_fs_state, current_fs_state)
                    # print(f"[GH] Detected {len(changed_files)} file changes")

                    # # Copy changed files to the container
                    # if changed_files and docker_manager is not None:
                    #     copy_files_to_container(docker_manager, self.fs_path, changed_files)
                    #     # Update initial state for next comparison
                    #     initial_fs_state = current_fs_state.copy()
            ...
            ## greenhouse_patch_flag: if True, use greenhouse patch logic to enhance firmwell
            ...
            if binary is not None and docker_manager is not None:
                # FW: If the binary was patched, copy it to the container
                container_bin_path = os.path.join("/fs", os.path.relpath(self.bin_path, self.fs_path))
                
                # Use helper function to copy binary
               
                
                os.chmod(self.bin_path, 0o755)
                docker_manager.docker_cp_to_container(self.bin_path, container_bin_path)
        
                if success:
                    print("[GH] Successfully copied patched binary to container using helper function")
                else:
                    print("[GH] Failed to copy binary using helper function, trying direct copy method")
    ...
    def copy_libnvram_faker_to_container(self, docker_manager, tempCont):
        """Copy libnvram-faker.so to the container"""
        if not docker_manager or not self.gh or not self.gh.fixer:
            print("[GH] Cannot copy libnvram-faker.so: docker_manager or gh.fixer not available")
            return False
        
        try:
            # Get library path info
            fs_path = self.fs_path
            lib_path = os.path.join(fs_path, "lib")
            arch = self.gh.fixer.arch
            clibc = self.gh.fixer.clibc
            nvram_faker_path = self.gh.fixer.nvram_faker_path
            
            # libnvram-faker.so source path
            source_nvram_faker_path = os.path.join(nvram_faker_path, "lib", arch, clibc, "libnvram-faker.so")
            os.chmod(source_nvram_faker_path, 0o755)
            
            # Verify source file exists
            if not os.path.exists(source_nvram_faker_path):
                print(f"[GH] Error: libnvram-faker.so does not exist: {source_nvram_faker_path}")
                return False
            
            print(f"[GH] Using {source_nvram_faker_path}")
            
            # Target file path in the container
            container_lib_path = "/fs/lib"
            container_nvram_faker_path = f"{container_lib_path}/libnvram-faker.so"
            
            
            # Copy libnvram-faker.so to the container
            print(f"[GH] Copying {source_nvram_faker_path} -> {container_nvram_faker_path}")
            docker_manager.docker_cp_to_container(source_nvram_faker_path, container_nvram_faker_path)

            container_lib_path = "/fs/usr/lib"
            container_nvram_faker_path = f"{container_lib_path}/libnvram-faker.so"
            ...
            print(f"[GH] Successfully copied libnvram-faker.so to container")
            return True
        
        except Exception as e:
            print(f"[GH] Error copying libnvram-faker.so: {e}")
            return False
...
def call_patch_loop(docker_ip,
                    ...
                    brand,
                    rehost_type,
                    greenhouse_patch_flag,
                    ):
    """init Greenhouse and run patch loop"""
    # args = get_cmdline_arg()
    # ff = Greenhouse(args)
    
    # Create Greenhouse instance
    # Construct args for Greenhouse
    class Args:
        def __init__(self):
            self.firmae = ""
            self.outpath = "/results"
            self.fs_path = new_fs
            self.target_bin = bin_path
            self.skipfile = None
            self.bintrunk = False
            self.max_depth = 5
            self.max_cycles = 25
            self.workspace = "/tmp/scratch"
            self.batchfolder = ""
            self.make_dot = False
            self.logpath = ""
            self.ip = docker_ip
            self.ports = "80,81"
            self.bin_args = new_run_args
            self.brand = brand
            self.partial_configs = False
            self.cache_path = "/cache"
            self.img_path = ""
            self.rehost_type = "HTTP"
            
            # Flags
            self.rehost_first = False
            self.norepeat = False
            self.diagnose = False
            self.nostrict = False
            self.nohack_bind = False
            self.nohack_devproc = False
            self.nohack_sysinfo = False
            self.nofullrehosting = True
            self.nocleanupfirmae = False
            self.baseline = False
            self.nodedaemon = True
            self.timeout = 7200
    ...
    ff.docker_ip = docker_ip
    ff.fs_path = new_fs
    ff.bin_path = bin_path
    ff.qemu_arch = qemu_arch
    ff.sha256hash = sha256hash
    
    ff.max_cycles = 25
    
    # some flag
    ff.rehost_first = False
    ff.skip_fullsystem = True
    ff.nodaemon = True
    ff.nopatch = False
    ff.rehost_type = rehost_type
    
    

    
    ff.fw_patch_loop(new_run_args,
                    cwdpath,
                    checker,
                    tempCont,
                    docker_manager,
